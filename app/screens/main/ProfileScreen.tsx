...
import { deleteMyAccount } from "../../services/users/profile.service";

export default function ProfileScreen() {
  const navigation = useNavigation<any>();
  const [showLogoutModal, setShowLogoutModal] = useState(false);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const { posts, fetchPosts, bookmarks, fetchBookmarks } = usePostStore();
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      await requestLogout(); // Clear tokens and notify backend
      navigation.reset({
        index: 0,
        routes: [{ name: "Auth", params: { screen: "Login" } }],
      });
          ...
          <View className="flex-row items-center mt-5 gap-8">
            <View className="items-center">
              <Text className="text-[18px] font-bold text-[#101828] font-parkinsans-bold">
                {(user._count?.followers ?? 0) >= 1000
                  ? ((user._count?.followers ?? 0) / 1000).toFixed(1) + "k"
